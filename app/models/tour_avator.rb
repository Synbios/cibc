class TourAvator < Tour
  ...
  def slice_content_image
    ...
    if original_path.present?
      ...
      system("convert #{original_path}  -crop 800x600  #{Rails.root}/public/ta_#{self.id}_%d.jpg")

      # turn slice images into shards
      file_paths = `ls #{Rails.root}/public/ta_#{self.id}_*.jpg`.split("\n")
      ...
      # remove tmp files
      system("rm #{Rails.root}/public/ta_#{self.id}_*.jpg")
    end
  end
end
